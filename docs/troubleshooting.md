...
3. **Beeline 실패** — `.env` `HIVE_SERVER2_JDBC` 가 CM ZK **`/default;keyStoreType=jks;principal=hive/ccycloud-1...;retries=5;...`** 형식인지; `./scripts/beeline.sh` / `./scripts/test_beeline_connect.sh`; [`03-cloudera-integration.md`](03-cloudera-integration.md)
